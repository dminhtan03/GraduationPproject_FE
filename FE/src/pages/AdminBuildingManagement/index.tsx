...
const AdminBuildingManagementPage: React.FC = () => {
  ...
  const [newBuilding, setNewBuilding] = useState<BuildingForm>({
    name: "",
    address: "",
    totalFloors: 1,
  });
  ...
  const loadBuildings = async () => {
    ...
    try {
      ...
      const normalizedBuildings = Array.isArray(data)
        ? data.map((item) => {
            const row = item as Record<string, unknown>;
            return {
              id: String(row.id ?? row.buildingId ?? ""),
              name: String(row.name ?? row.buildingName ?? ""),
  ...
  const handleCreateBuilding = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg = getErrorMessage(err, "Failed to create building");
  ...
  const handleUpdateBuilding = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg = getErrorMessage(err, "Failed to update building");
      setToastPopup({ type: "error", message: errorMsg });
    }
  };
